    ...
    <div className="flex flex-col gap-20 pb-20">
        ...
        <div className="container relative z-10 px-4 text-center max-w-4xl mx-auto">
          ...
          >
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary/10 text-primary font-medium mb-6 border border-primary/20 backdrop-blur-md">
              ...
              {lang === 'ar' ? 'أفضل متجر للمنتجات الرقمية' : 'Premium Digital Store'}
            </div>
            <h1 className="text-5xl md:text-7xl font-bold mb-6 text-foreground leading-tight tracking-tight">
              {lang === 'ar' ? 'عالم من الترفيه' : 'A World of Entertainment'} <br />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-secondary">
                {lang === 'ar' ? 'بين يديك' : 'At Your Fingertips'}
              </span>
            </h1>
            <p className="text-lg md:text-xl text-muted-foreground mb-10 max-w-2xl mx-auto">
              {lang === 'ar' 
                ? 'اشتراكات، بطاقات هدايا، وألعاب. تسليم فوري وآمن وبأفضل الأسعار في البحرين.' 
                : 'Subscriptions, gift cards, and games. Instant secure delivery with the best prices in Bahrain.'}
            </p>
      ...
      <section className="container mx-auto px-4 -mt-16 relative z-20">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[
            { icon: Zap, title: lang === 'ar' ? 'تسليم فوري' : 'Instant Delivery', desc: lang === 'ar' ? 'استلم طلبك رقمياً فور الدفع' : 'Get your order digitally right after payment' },
            { icon: Shield, title: lang === 'ar' ? 'دفع آمن' : 'Secure Payment', desc: lang === 'ar' ? 'بوابات دفع مشفرة وموثوقة' : 'Encrypted and trusted payment gateways' },
            { icon: Clock, title: lang === 'ar' ? 'دعم 24/7' : '24/7 Support', desc: lang === 'ar' ? 'فريق جاهز لخدمتك دائماً' : 'Our team is always ready to help' },
          ].map((feature, i) => (
            <motion.div 
              key={i}
      ...
      <section className="container mx-auto px-4">
        <div className="bg-gradient-to-r from-primary to-secondary rounded-3xl p-10 md:p-16 text-white flex flex-col md:flex-row items-center justify-between gap-8 relative overflow-hidden shadow-2xl">
          ...
          <div className="relative z-10 max-w-2xl text-center md:text-start">
            <h2 className="text-3xl md:text-5xl font-bold mb-4">{lang === 'ar' ? 'احصل على خصم 10% على أول طلب' : 'Get 10% Off Your First Order'}</h2>
            <p className="text-lg text-white/80 mb-8">{lang === 'ar' ? 'استخدم الكود WELCOME10 عند الدفع واستمتع بأفضل المنتجات الرقمية' : 'Use code WELCOME10 at checkout and enjoy the best digital products'}</p>
